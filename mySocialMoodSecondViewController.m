//
//  mySocialMoodSecondViewController.m
//  mySocialMood
//
...
#import "mySocialMoodSecondViewController.h"
...
@interface mySocialMoodSecondViewController (){
...
@implementation mySocialMoodSecondViewController{
...
-(void)handleEvents:(NSNotification *) notification{
    ...
    if([operation isEqualToString:@"locations Loaded"]){
        [self performSelector:@selector(testSticker) withObject:nil afterDelay:0.2];
        globalSingleton* globals = [globalSingleton sharedManager];
        globeViewC.height = 0.4;

        [globeViewC animateToPosition:MaplyCoordinateMakeWithDegrees([globals my_longitude], [globals my_latitude]) time:3.0];
    }
    
    if([operation isEqualToString:@"config changed"]){
        [self editDone];
    }
} 
...
- (void)viewDidLoad
{
    ...
    configViewC = [storyboard instantiateViewControllerWithIdentifier:@"maply_config"];
    ...
    */
    ...
    [button setTitle:@"Show Options" forState:UIControlStateNormal];
    button.frame = CGRectMake(5.0, 5.0, 160.0, 40.0);
...
- (void)addLabels:(LocationInfo *)locations len:(int)len stride:(int)stride offset:(int)offset
{
    ...
    for (unsigned int ii=offset;ii<len;ii+=stride)
    {
        ...
        label.size = size;
        label.text = [NSString stringWithFormat:@"%s",location->name];
        label.userObject = [NSString stringWithFormat:@"%s",location->details];
        [labels addObject:label];
    }
...
{
    UIImage *startImage = [UIImage imageNamed:@"smile_small"];
...
- (void)showConfig
{
    if (UI_USER_INTERFACE_IDIOM() ==  UIUserInterfaceIdiomPad)
    {
        ...
        [self presentViewController:configViewC animated:YES completion:nil];
    }
}

-(void)testSticker{
    globalSingleton* globals_ = [globalSingleton sharedManager];
    //LocationInfo locations[[[globals_ world_locations] count]];
    int index=0;
    for (NSMutableArray*loc in [globals_ world_locations]) {
        float latitude = [[loc objectAtIndex:2] floatValue];
        float longitude = [[loc objectAtIndex:3] floatValue];
        memcpy(locations[index].name, [[loc objectAtIndex:0] UTF8String], 20);
        
        locations[index].lat = latitude;
        locations[index].lon = longitude;
        
        
        
        memcpy(locations[index].details, [[[loc objectAtIndex:4] stringByReplacingOccurrencesOfString:@"<br>" withString:@"\n"] UTF8String], 256);
        
        index++;
    }
    int num = [[globals_ world_locations] count]+1;
    [self addStickers:locations len:num stride:1 offset:0 desc:@{kMaplyFade: @(1.0)}];
    [self addLabels:locations len:num stride:1 offset:0];
}
...
- (void)handleSelection:(NSObject *)selectedObj
{
    ...
    } else if ([selectedObj isKindOfClass:[MaplyLabel class]])
    {
        ...
        loc = label.loc;
        msg = [NSString stringWithFormat:@"%@",label.userObject];
